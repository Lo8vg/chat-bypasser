...
-- Chat mode: "Here" = local server, "Global" = cross-server
local chatMode = "Here"
...
sendButton.Size = UDim2.new(0, 45, 0, 32)
...
-- Chat Mode Toggle Button (Here/Global)
local chatModeButton = Instance.new("TextButton")
chatModeButton.Size = UDim2.new(0, 50, 0, 32)
chatModeButton.Position = UDim2.new(0, 48, 0, 0)
chatModeButton.BackgroundColor3 = Color3.fromRGB(60, 120, 60)
chatModeButton.TextColor3 = Color3.fromRGB(255, 255, 255)
chatModeButton.Text = "Here"
chatModeButton.Font = Enum.Font.GothamBold
chatModeButton.TextSize = 10
chatModeButton.Parent = bottomRow
local chatModeCorner = Instance.new("UICorner")
chatModeCorner.CornerRadius = UDim.new(0, 6)
chatModeCorner.Parent = chatModeButton

local delayTextbox = Instance.new("TextBox")
delayTextbox.Size = UDim2.new(0, 30, 0, 32)
delayTextbox.Position = UDim2.new(0, 101, 0, 0)
...
delayLabel.Position = UDim2.new(0, 133, 0, 0)
...
spamButton.Size = UDim2.new(0, 65, 0, 32)
spamButton.Position = UDim2.new(1, -65, 0, 0)
...
chatModeButton.MouseButton1Click:Connect(function()
    if chatMode == "Here" then
        chatMode = "Global"
        chatModeButton.Text = "Global"
        chatModeButton.BackgroundColor3 = Color3.fromRGB(120, 60, 160)
    else
        chatMode = "Here"
        chatModeButton.Text = "Here"
        chatModeButton.BackgroundColor3 = Color3.fromRGB(60, 120, 60)
...
-- Helper function to get the appropriate chat channel
local function getChatChannel()
    local channel = TextChatService:FindFirstChild("TextChannels")
    if not channel then return nil end
    
    if chatMode == "Global" then
        -- Try to find the Global channel (cross-server)
        local globalChannel = channel:FindFirstChild("RBXGlobal")
        if globalChannel then return globalChannel end
        -- Fallback: try alternate naming conventions
        globalChannel = channel:FindFirstChild("Global")
        if globalChannel then return globalChannel end
    end
    
    -- Default to Here (local server) channel
    local hereChannel = channel:FindFirstChild("RBXGeneral")
    return hereChannel
end
...
local function sendMessage(msg)
    ...
    -- Try TextChatService first (newer system with Global support)
    ...
    -- Fallback to legacy chat system (only supports Here/local)
...
local function sendBackup1Message(msg)
    ...
    -- Uses the same chat mode as main chat
    if TextChatService then
        local chatChannel = getChatChannel()
        if chatChannel then
            chatChannel:SendAsync(message)
...
print("✅ Custom Chat GUI Loaded - Chat Mode: Here/Global toggle available")
