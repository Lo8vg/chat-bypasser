...
-- Textbox Container (ScrollingFrame)
...
-- AUTO-SCROLL FUNCTION (instant, no delay)
local function autoScroll(container)
    task.spawn(function()
        container.CanvasPosition = Vector2.new(0, 999999)
    end)
end

-- SCROLL ON TEXT CHANGE (instant)
textbox:GetPropertyChangedSignal("Text"):Connect(function()
    autoScroll(textboxContainer)
end)

backup1Textbox:GetPropertyChangedSignal("Text"):Connect(function()
    autoScroll(backup1Container)
end)

backup2Textbox:GetPropertyChangedSignal("Text"):Connect(function()
    autoScroll(backup2Container)
end)
...
-- QUICK CHAT: Enter key sends and clears instantly, keeps focus
...
print("✅ Custom Chat GUI Loaded (PC Quick Chat)")
